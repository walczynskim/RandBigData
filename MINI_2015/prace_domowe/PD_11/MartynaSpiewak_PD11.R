...
f2 <- function(){
   ...
   inda <- lapply(1:500, function(i){
      ...
      resx <- as.numeric(l[1])
      resy <- as.numeric(l[2])
      c(resx = resx, resy = resy, ind)})
   
   # posortuj wartosci w kazdym wierszu
   df2 <- do.call("rbind", inda)
   res <- apply(df2, 1, function(x) x[order(x)])
}

microbenchmark::microbenchmark(f1(), f2(), times = 1, unit = "relative")
# Unit: relative
# expr      min       lq     mean   median       uq      max neval
# f1() 8.967186 8.967186 8.967186 8.967186 8.967186 8.967186     1
# f2() 1.000000 1.000000 1.000000 1.000000 1.000000 1.000000     1
