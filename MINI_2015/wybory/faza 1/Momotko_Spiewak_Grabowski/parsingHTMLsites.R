...
parse_natemat.pl<-function(){
   ...
   #PODGLOWNA - 3
   podglowna <- glowna
   podglowna_text<-stri_trim_both(stri_replace_all_regex(xml_text(podglowna,"alt"),"(\\n)|(\\t)|(\\r)|(\")|(;)"," "))
   niepotrzebne<-which(podglowna_text=="")
   podglowna_text<-podglowna_text[-niepotrzebne]
   niepotrzebne2<-stri_detect_regex(podglowna_text,"Wi?cej zdj?? \\([0-9]")
   ...
   #BOK - 2
   bok<-podglowna
   bok_text<-stri_trim_both(stri_replace_all_regex(xml_text(bok,"alt"),"(\\n)|(\\t)|(\\r)|(\")|(;)"," "))
   niepotrzebne<-which(bok_text=="")
   bok_text<-bok_text[-niepotrzebne]
   niepotrzebne2<-stri_detect_regex(bok_text,"Wi?cej zdj?? \\([0-9]")
   ...
   niepotrzebne2<-stri_detect_regex(dol_text,"Wi?cej zdj?? \\([0-9]")
   ...
   if(sum(ktore)>0){
      ...
      natemat<-rbind(natemat, data.frame(date=data, portal=url, title=dol_text[ktore], position=1,link=dol_link[ktore]))
   }
...
parse_tvp.info<-function(){
   ...
   tytuly_prawy<-stri_replace_all_regex(tytuly_prawy, ';', "")
   tytuly_prawy<-stri_trim_both(stri_replace_all_regex(tytuly_prawy,"(\\n)|(\\t)|(\\r)|(\")"," "))
   tytuly_prawy<-stri_trim_both(tytuly_prawy)[seq(1, length(tytuly_prawy), by=2)]
...
slownik<-slowa_klucz<-c("bronisław","bronisława", "komorowski","komorowskiego","komorowskiemu",
                        "bronislaw",
...
arch_tvn<-function(){
   ...
   for(j in 1:n){
      ...
      if(sum(ktore)>0){
         tvn<-rbind(tvn,data.frame(date=daty[ktore], portal="http://www.tvn24.pl/",
                                   title=info_text[ktore], position=2,link=info_linki[ktore]))
...
wpolityce<-function(n=1){
   
   wygenerowane<-character(n)
   
   for(i in 1:n){
      wygenerowane[i]<-stri_paste("http://wpolityce.pl/szukaj?q=wybory%20prezydenckie&page=",i)
   }
   ...
   for(i in 1:n){
      ...
      teksty<-stri_trim_both(stri_replace_all_regex(html_text(html_nodes(doc,"h3 a")),"(\\n)|(\\t)|(\\r)|(\")|(;)"," "))
      #linki
      ...
      if(sum(ktore)>0){
         ...
         polityka<-rbind(polityka, data.frame(date=daty[ktore], portal=url,
                                              title=teksty[ktore], position=2,link=linki[ktore])) 
      }
