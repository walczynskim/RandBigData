...
id.kandydant.all<-list(
  ...
  jacek_wilk = c("499646073441042", "415099301976549", "335606443297037", "884935904861907"),
  grzegorz_braun = c("368902689980960",  "973776299316688", "423516244481713", "1538401599758196"))
...
facebook<-function(id.kandydant.all){
  ...
  for(x in id.kandydant){
    kandydant<-rbind(kandydant, tryCatch(getPage(page=x, token=fb_oauth, 
                                                 ...
                                                 until = stri_replace_all_regex(as.character(Sys.Date()), "-", "/"), feed=FALSE, n=500),
...
# aktulizacja wskaznikow, okreslajacych widocznosc kandydatow:

setwd("C:\\Users\\MARTYNKA\\Desktop\\R i BIG DATA\\funkcje")
source("facebook_wskazniki.R")
facebook_wskazniki()
