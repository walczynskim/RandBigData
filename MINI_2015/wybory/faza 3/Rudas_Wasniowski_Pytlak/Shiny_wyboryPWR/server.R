...
dane_artykuly<-read.table("projekt_wybory_R_i_big_data/artykuly/artykuly.txt",
...
######### wskazniki dla oficjanych tweeterow kandydatow
   ...
   output$wykres_ilosc_ofic <- renderPlot({
      ...
      s<-"projekt_wybory_R_i_big_data/twitter/ilosc_ofic"
      wykres_ilosc_ofic(s, 
                        data_od=input$od_do_ofic[1],
   ...
   output$wykres_ulubione_ofic <- renderPlot({
      if (length(input$kandydaci_ofic)==0) return(NULL)
      s<-"projekt_wybory_R_i_big_data/twitter/ulubione_ofic"
      wykres_ulubione_ofic(s, 
                        data_od=input$od_do_ofic[1],
                        data_do=input$od_do_ofic[2],
                        kandydaci=input$kandydaci_ofic)
   })
   ...
   output$wykres_retweety_ofic <- renderPlot({
      if (length(input$kandydaci_ofic)==0) return(NULL)
      s<-"projekt_wybory_R_i_big_data/twitter/retweety_ofic"
      wykres_retweety_ofic(s, 
                           data_od=input$od_do_ofic[1],
                           data_do=input$od_do_ofic[2],
                           kandydaci=input$kandydaci_ofic)
   })
   ...
   output$wykres_wydzwiek_ofic <- renderPlot({
      if (length(input$kandydaci_ofic)==0) return(NULL)
      s<-"projekt_wybory_R_i_big_data/twitter/wydzwiek_ofic"
      wykres_wydzwiek_ofic(s, 
                           data_od=input$od_do_ofic[1],
                           data_do=input$od_do_ofic[2],
                           kandydaci=input$kandydaci_ofic)
   })
 ...
 #### wskazniki dla nasluchu twittera
   ...
   output$wykres_ilosc_nasluch <- renderPlot({
      ...
      s<-"projekt_wybory_R_i_big_data/twitter/nasluch_ilosc"
      wykres_ilosc_nasluch(s, 
                        data_od=input$od_do_nasluch[1],
   ...
   output$wykres_wydzwiek_nasluch <- renderPlot({
      if (length(input$kandydaci_nasluch)==0) return(NULL)
      s<-"projekt_wybory_R_i_big_data/twitter/nasluch_wydzwiek"
      wykres_wydzwiek_nasluch(s, 
                           data_od=input$od_do_nasluch[1],
                           data_do=input$od_do_nasluch[2],
                           kandydaci=input$kandydaci_nasluch)
   })
