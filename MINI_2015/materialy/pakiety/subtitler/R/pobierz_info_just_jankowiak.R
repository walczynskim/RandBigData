...
info.imdb <- function(id) {
    ...
    lista.imdb = list(id = id_film, link = url.imdb, Title = tytul, Year = rok, Time = czas, Relase = data_wydania, Genre = gatunek, 
        Storyline = fabula, Country = kraj, Ratings = ocena, Ratings_max = max_ocena, Users = liczba_glosujacych, Reviews_number = liczba_recenzji, 
        Budget = budzet, Keywords = klucze, Music = muzyka, Produced = producenci)
...
get_names <- function(www) {
    ...
    if (is.character(url) && url == "NA") 
        return("NA")
    prod <- tryCatch({
        url %>% html_nodes(".simpleCreditsTable:nth-child(9) a") %>% html_text() %>% stri_trim_both()
    }, error = function(e) {
        "NA"
    })
    music <- tryCatch({
        url %>% html_nodes(".simpleCreditsTable:nth-child(11) a") %>% html_text() %>% stri_trim_both()
    }, error = function(e) {
        "NA"
    })
    if (length(prod) == 0) 
...
info.wiki <- function(tytul, rok) {
    ...
    jakie_info <- c("Gatunek", "Kraj produkcji", "Jêzyk", "Czas trwania", "Re¿yseria", "Bud¿et")
    w <- match(jakie_info, tabela_tekst)
    info <- data.frame(gatunek = character(0), kraj_produkcji = character(0), jezyk = character(0), czas_trwania = character(0), 
        rezyseria = character(0), budzet = character(0), stringsAsFactors = FALSE)
    info <- rbind(info, data.frame(gatunek = ifelse(is.na(w[1]), NA, tabela_tekst[w[1] + 1]), kraj_produkcji = ifelse(is.na(w[2]), 
        NA, tabela_tekst[w[2] + 1]), jezyk = ifelse(is.na(w[3]), NA, tabela_tekst[w[3] + 1]), czas_trwania = ifelse(is.na(w[4]), 
        NA, tabela_tekst[w[4] + 1]), rezyseria = ifelse(is.na(w[5]), NA, tabela_tekst[w[5] + 1]), budzet = ifelse(is.na(w[6]), 
        NA, tabela_tekst[w[6] + 1]), stringsAsFactors = FALSE))
    return(t(info))
}
