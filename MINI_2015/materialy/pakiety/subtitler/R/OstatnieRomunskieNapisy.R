...
OstatnieRomunskieNapisy <- function(movieYear = 2015, imdbGrade = 5, fileSave = FALSE, hd = FALSE) {
    ...
    moviesNames <- html_nodes(htmlSite, ".bnone") %>% html_text() %>% stri_replace_all_regex(., "\n \t\t\t|;", "")
    moviesDownloadLinks <- html_nodes(htmlSite, "#search_results td:nth-child(5) a") %>% html_attr("href") %>% stri_paste("http://www.opensubtitles.org", 
        .)
