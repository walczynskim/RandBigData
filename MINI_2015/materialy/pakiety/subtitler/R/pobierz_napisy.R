...
pobierz_napisy <- function(tytul, jezyk) {
    ...
    # wczytanie wyszukanych na stronie url linkow i tytulow filmow. Funkcje wykonamy, jesli jakiekolwiek sie znalazly.
    hrefs_titles <- linki_i_tytuly_na_stronie(url)
    if (length(hrefs_titles) > 0) {
        ktory_tytul <- hrefs_titles["title", ]
        # Co jesli uzytkownik podal film 'Shrek' a program nie wie, czy chodzilo mu o Shrek - Shrek, czy 'Shrek Forever' czy 'Shrek
        # 2'?
        ...
        if (!is.null(pobierz_attrs)) {
            ...
            # Normalnie napisy powinny pobrac sie juz w tym kroku, jednak czesto na stronie opensubtitles.org trzeba kliknac w jeszcze
            # jeden link np. dla filmow 'La vita e bella' lub 'Titanic'. Jezeli atrybuty pobierania w tym kroku ustawione sa na NULL
            # (pomimo, ze stri_detektor wykryl film) nalezy wykonac jeszcze jedno przejscie wglab strony.
        } else {
            ...
            # czy_znaleziono_tytul <- stri_detect_regex(tolower(href_n_title[2,]), tolower(tytul)) nie trzeba sprawdzac, czy znaleziono
            # tytul, bo na to juz wskazuje sam fakt ze sie wyszukalo.
...
pobierz_rozpakuj <- function(pobierz_attrs) {
    ...
    if (dir_name %in% dir()) 
        message("Uwaga, nadpisanie istniejacych plikow!") else dir.create(dir_name)
