...
stat_czas <- function(link, sciezka) {
    ...
    ggplot(data.frame(indeks = indeks, czas = dlg_czas), aes(x = indeks, y = czas)) + geom_smooth(stat = "smooth") + geom_point(aes(x = indeks, 
        y = czas)) + xlab("czas filmu(hh:mm:ss)") + ylab("czas(s)") + scale_x_continuous(breaks = indeks[w], labels = os_x[w])
