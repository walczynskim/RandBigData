...
zapisz <- function( co, gdzie ){
      if( nrow(co) ){
            if( file.exists(gdzie) )
                  write.table(co, file=gdzie, append=TRUE, col.names = FALSE, row.names = FALSE) else
                        write.table(co, file=gdzie, row.names = FALSE)
      }
}
...
fb_posty_i_komentarze <- function( kto, z_ilu_dni_temu=1 ){
      ...
      for(page in strony_kandydata){
            wpisy <- rbind( wpisy, tryCatch(
                  getPage(page=page, token=fb_oauth,
                          ...
                          until = stri_replace_all_regex(as.character( Sys.Date()-1 ), "-", "/"), 
                          feed=FALSE),
      ...
      sciezka <- paste0("Facebook/", kto, "_facebook", ".csv")
      ...
      sciezka <- paste0("Facebook/", kto, "_facebook_comments", ".csv")
...
#       require(Rfacebook)
#       load("fb_oauth")  # contains fb_oauth data and id.kandydat.all as well.
      ...
      sciezka <- paste0("Facebook/Likes/", kto, "_likes", ".csv")
...
fb_sesja <- function(z_ilu_dni_temu=1){
      require(stringi)
      require(Rfacebook)
      load("fb_oauth")  # zawiera rowniez uzywany pozniej obiekt: id.kandydant.all
